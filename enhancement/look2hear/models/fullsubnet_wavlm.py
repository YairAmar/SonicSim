...
class FullSubnetWavLM(BaseModel):
    def __init__(
        self,
        num_freqs,
        look_ahead,
        ...
        weight_init=True,
        sample_rate=16000, 
        use_upstream_model=True,
        upstream_model="wavlm",
        upstream_model_path=None,
        ...
        self.upstream_interface = FeatureFusionInterface(
            ...
            upstream_feature_space=upstream_feature_space,
            downstream_feature_space=downstream_feature_space,
            downstream_timesteps=downstream_timesteps,
            fusion_residual=fusion_residual,
            fusion_use_conv=fusion_use_conv,
            aggregation_layer_wise=aggregation_layer_wise
        )
    ...
    def forward(self, noisy):
        ...
        upstream_x = self.upstream_model(noisy)
        sb_input_shape = sb_input.shape
        sb_input = sb_input.reshape(
            sb_input_shape[0], sb_input_shape[1] * sb_input_shape[2], sb_input_shape[3]
        )
        sb_input = self.upstream_interface(upstream_x, sb_input)
        sb_input = sb_input.reshape(
            sb_input_shape[0], sb_input_shape[1], sb_input_shape[2], sb_input_shape[3]
        )
